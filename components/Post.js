...
import { HeartIcon as HeartIconFilled,  ChatIcon as ChatIconFilled,} from "@heroicons/react/solid";  
  ...
  const Post = ({ id, post, postPage }) => {
     ...
     useEffect(() => setLiked(likes.findIndex((like) => like.id === session?.user?.uid) !== -1), 
      ...
      //When picture is cliked twice pic is going to be desliked
      const likePost = async () =>{
         if(liked){
             await deleteDoc(doc(db,"posts",id,"likes",session.user.uid))
         }else{
             //also creating a collection of likes
  ...
  return (
    //onClick={() => router.push(`/${id}`)} -> takes you to the postpage
         ...
         <div className={`flex ${!postPage && "justify-between"}`}>
              ...
              <img 
               ...
               <div className={`text-[#6e767d] flex justify-between w-10/12 ${postPage && "mx-auto"}`}>
                ...
                {/** Icons */}
                {/** -> setPostId(id) is made to be accesed globaly so we can know which post we are referencing to 
                 * when conmmenting.
                 * ->setIsOpen(true) (from model state) onclick it has to be open to be able to comment.
                */}
                {/**Comment button */}
         <div
          ...
          >
            ...
            {comments.length > 0 && (
             ...
             {/** Delete button */}
             {/** {session.user.uid === post?.id ? ->This basicaly mean if its not you in your session you cannot
              * delete another persons posts
              */}
